			...
			while jT > 0 and not eJE do
				...
				if D > 90 then
					...
					if D > 100 then
						...
						TriggerEvent('chatMessage', '[^8Nice try!^)]', { 0, 0, 0 }, "Your jail time was extended for an unlawful escape attempt.")
					end
				end
				jT = jT - 0.5
			end
			TriggerServerEvent('chatMessageEntered', "[^8Jailer^0]", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." has been released from jail.")
